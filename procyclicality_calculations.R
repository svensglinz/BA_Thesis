...
start_all <- as.Date("2006-01-01")
end_all <- as.Date("2021-01-01")
start_covid <- as.Date("2020-01-01")
end_covid <- as.Date("2020-12-31")
start_fc <- as.Date("2008-01-01")
end_fc <- as.Date("2008-12-31")
start_dotcom <- as.Date("1999-01-01")
end_dotcom <- as.Date("2001-01-01")
...
for (i in lambda_loop) {
    ...
    cap <- quantile(margin_floor$MARGIN, .99, na.rm = TRUE)[[1]]
    ...
    # speed & floor
    limit <- quantile(margin_baseline$MARGIN / lead(margin_baseline$MARGIN, 20), .95, na.rm = TRUE)[[1]]
...
write_csv(measures, "Data/procyclicality_calculations.csv")
